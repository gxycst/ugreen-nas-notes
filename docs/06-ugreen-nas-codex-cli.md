...
### 3. 绿联桌面的 Codex 快捷方式：这台 NAS 的实际做法

这台 NAS 的 **Codex** 图标最初是在 UGOS Pro 的 **Docker → ttyd 容器 → `···` → 桌面快捷方式** 中创建的。当时表单默认显示端口 `7681`、名称 `TTYD`；将名称改为 `Codex` 后添加。后来把 Docker 版 ttyd 改为宿主机上的 `codex-web.service`，仍由宿主机监听 `7681`。原有快捷方式保留下来，点击图标可以继续打开 Codex。

因此，新装时不要只执行 `systemctl enable --now codex-web` 就期待桌面自动出现图标。先用 `http://<NAS_IP>:7681` 验证服务；如果 Docker 界面中仍有提供快捷方式入口的相关容器，再从该容器的 `··· → 桌面快捷方式` 创建，端口填 `7681`、名称填 `Codex`，然后在局域网和 UGREENlink 下分别测试。**这一步依赖 UGOS 的 Docker 快捷方式入口；本次没有验证“没有相关容器时，直接给宿主机任意端口新建快捷方式”的方法。**

现有 Codex 图标能正常使用时，保留它即可。不要为了重建图标删除现有快捷方式；原先绑定的 Docker 容器已不在，新建入口是否还能复现需要单独验证。

`7681` 是没有独立身份验证的 HTTP 终端。公网使用应经过 UGREENlink 登录层、Tailscale 或其他有认证的入口，不要直接做路由器端口映射。
